...
public class DDBlockLoot extends BlockLootSubProvider {
    ...
    protected void generate() {
        ...
        add(DDBlocks.MOB_PLACER.get(), LootTable.lootTable());
        add(DDBlocks.SCORCHER_LIGHT_BLOCK.get(), LootTable.lootTable());
        add(DDBlocks.MOSSY_GRIMESTONE.get(), block -> createSingleItemTableWithSilkTouch(DDBlocks.MOSSY_GRIMESTONE.get(), DDBlocks.GRIMESTONE.get().asItem()));
    ...
    private void autoGenerateDrops() {
        ...
        defaultDropBlocks.forEach(block -> {
            if (block instanceof SlabBlock b) {
                add(b, createSlabItemTable(b));
            } else if (block instanceof VerticalSlabBlock b) {
                add(b, createSlabItemTable(b));
            } else if (block instanceof DoorBlock b) {
                add(b, createDoorTable(b));
            } else if (block instanceof BushBlock b) {
                add(b, createGrassDrops(b));
            } else if (block instanceof FlowerPotBlock b) {
                add(b, createPotFlowerItemTable(b.getContent()));
            } else {
                dropSelf(block);
            }
        });
