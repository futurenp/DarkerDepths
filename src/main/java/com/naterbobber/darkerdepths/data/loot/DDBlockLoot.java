...
public class DDBlockLoot extends BlockLootSubProvider {
    ...
    protected void generate() {
        ...
        add(DDBlocks.SHELF_GLOWSHROOM.get(), block -> createBooleanBasedLoot(DDBlocks.SHELF_GLOWSHROOM.get(), DDBlockStateProperties.LARGE));

        add(DDBlocks.GULBS.get(), createGrassDrops(DDBlocks.GULBS.get()));
    ...
    private void autoGenerateDrops() {
        DDBlockSets.WOOD_BLOCK_SETS.forEach(blockSet -> {
            add(blockSet.getWallSign().get(), block ->
                    createSingleItemTable(blockSet.getSign()));

            add(blockSet.getWallHangingSign().get(), block ->
                    createSingleItemTable(blockSet.getHangingSign()));

            add(blockSet.getBookshelf().get(), block ->
                    createSingleItemTableWithSilkTouch(blockSet.getBookshelf().get(), Items.BOOK, ConstantValue.exactly(3.0F)));
        });
    ...
    private LootTable.Builder createBooleanBasedLoot(Block block, BooleanProperty booleanProperty) {
        return LootTable.lootTable()
                .withPool(LootPool.lootPool()
                        .setRolls(ConstantValue.exactly(1.0F))
                        .add(LootItem.lootTableItem(block)
                                .when(LootItemBlockStatePropertyCondition.hasBlockStateProperties(block)
                                        .setProperties(StatePropertiesPredicate.Builder.properties()
                                                .hasProperty(booleanProperty, false)
                                        )
                                )
                        ))
                .withPool(LootPool.lootPool()
                        ...
                        .add(LootItem.lootTableItem(block)
                                .when(LootItemBlockStatePropertyCondition.hasBlockStateProperties(block)
                                        .setProperties(StatePropertiesPredicate.Builder.properties()
                                                .hasProperty(booleanProperty, true)
                                        )
                                )
                        ));
    }
