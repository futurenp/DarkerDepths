...
public class MossyGrimestoneBlock extends Block implements BonemealableBlock {
    ...
    public TriState canSustainPlant(BlockState state, BlockGetter level, BlockPos soilPosition, Direction facing, BlockState plant) {
        if(plant.is(Blocks.SUGAR_CANE)) {
            for (Direction direction : Direction.Plane.HORIZONTAL) {
                var hPos = soilPosition.relative(direction);
                var hState = level.getBlockState(hPos);
                var hFluidstate = level.getFluidState(hPos);
                if (state.canBeHydrated(level, soilPosition, hFluidstate, hPos) || hState.is(Blocks.FROSTED_ICE)) {
        ...
        else if(plant.is(Blocks.CACTUS)) {
            return TriState.FALSE;
        }
