    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
        ...
        for (int i = 0; i < tries; i++) {
            ...
            if (world.getBlockState(mutableBlockPos).is(BlockTags.BASE_STONE_OVERWORLD) && world.isStateAtPosition(mutableBlockPos, DripstoneUtils::isEmptyOrWater)) {
                world.setBlock(mutableBlockPos, DDBlocks.GLOWSHROOM.get().defaultBlockState().setValue(BlockStateProperties.WATERLOGGED, world.getFluidState(mutableBlockPos).getType() == Fluids.WATER).setValue(DDBlockStateProperties.GLOWSHROOM_CLUSTERS, 1), 2);
            } else {
                break;
            }
        }
