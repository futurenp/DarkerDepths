...
public class ChalkPillarFeature extends Feature<LargeDripstoneConfiguration> {
    public ChalkPillarFeature(Codec<LargeDripstoneConfiguration> codec) {
        super(codec);
    }
        ...
        if (!DripstoneUtils.isEmptyOrWater(worldgenlevel.getBlockState(blockpos))) {
            ...
            if (!optional.isEmpty() && optional.get() instanceof Column.Range) {
                ...
                if (column$range.height() < 4) {
                    ...
                    ChalkPillarFeature.MoltenPillar largedripstonefeature$largedripstone = makeDripstone(
                    ...
                    ChalkPillarFeature.MoltenPillar largedripstonefeature$largedripstone1 = makeDripstone(
                    ...
                    ChalkPillarFeature.WindOffsetter largedripstonefeature$windoffsetter;
                    if (largedripstonefeature$largedripstone.isSuitableForWind(largedripstoneconfiguration)
                        ...
                        largedripstonefeature$windoffsetter = new ChalkPillarFeature.WindOffsetter(
                                blockpos.getY(), randomsource, largedripstoneconfiguration.windSpeed
                        );
                    } else {
                        largedripstonefeature$windoffsetter = ChalkPillarFeature.WindOffsetter.noWind();
                    }
    ...
    static final class MoltenPillar {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel level, ChalkPillarFeature.WindOffsetter windOffsetter) {
            while (this.radius > 1) {
        ...
        void placeBlocks(WorldGenLevel level, RandomSource random, ChalkPillarFeature.WindOffsetter windOffsetter) {
                        ...
                        if (k > 0) {
                            ...
                            for (int i1 = 0; i1 < k && blockpos$mutableblockpos.getY() < l; i1++) {
                                ...
                                if (DripstoneUtils.isEmptyOrWaterOrLava(level.getBlockState(blockpos))) {
                                    flag = true;
                                    Block block = DDBlocks.BLUE_CHALK.get();
                                    level.setBlock(blockpos, block.defaultBlockState(), 2);
                                } else if (flag && level.getBlockState(blockpos).is(DDTags.Blocks.CHALK_STRIPE_REPLACEABLE)) {
                                    break;
                                }
    ...
    static final class WindOffsetter {
        ...
        static ChalkPillarFeature.WindOffsetter noWind() {
            return new ChalkPillarFeature.WindOffsetter();
        }
